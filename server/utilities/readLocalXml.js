		...
		parser.parseString(data, function (err, results) {
			...
			// Iterate over emissions and add them all to an array
				...
				programmes.forEach(function(cur, i, arr) {
					...
					// I've recently changed the start and end from a moment to a number, and made start and end Number
					var startTime = checkRollover(moment.utc([year, month, day, startHour, startMinute ]), i).valueOf()
					var endHour = hrMin(cur.end)[0]
					var endMinute = hrMin(cur.end)[1]
					var endTime = checkRollover(moment.utc([year, month, day, endHour, endMinute]), i).valueOf()
